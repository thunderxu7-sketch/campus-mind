...
`migrations/001_initial.sql` 是 PostgreSQL 参考迁移，包含租户复合外键、跨实体操作者/审批者关系约束、敏感数据最小字段、会话撤销、无手机学生一次性凭证、名单导入预检哈希、普通学年频次唯一约束、专业复评例外、媒体资源关联、带用途的导出任务和行级安全策略。所有业务表均启用并 `FORCE` RLS（当前共 36 张表）；当前默认本地运行使用加密 JSON 适配器，因此不需要 Docker 或外部数据库。
