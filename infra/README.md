...
`migrations/001_initial.sql` 是 PostgreSQL 参考迁移，包含租户复合外键、敏感数据最小字段、普通学年频次唯一约束、专业复评例外、媒体资源关联和行级安全策略。所有业务表均启用并 `FORCE` RLS；当前默认本地运行使用加密 JSON 适配器，因此不需要 Docker 或外部数据库。
