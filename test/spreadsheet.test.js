...
import { parseCsv } from '../dist/apps/api/src/domain/service.js';
...
test('CSV reader preserves quoted commas and multiline cells within bounds', () => {
  const rows = parseCsv('externalId,displayName,age,classId\n"csv-001","合成,学生\n甲",15,class-demo-1');
  assert.deepEqual(rows, [{ externalId: 'csv-001', displayName: '合成,学生\n甲', age: '15', classId: 'class-demo-1' }]);
  assert.throws(() => parseCsv('externalId,displayName\n"unterminated,student'), /引号未闭合/);
});
