...
test('synthetic scales are barred only in production mode', () => {
  ...
  const expiredLicensed = { ...scale, provenance: 'licensed', licenseExpiresAt: '2020-01-01T00:00:00.000Z' };
  assert.throws(() => assertUsableScale(expiredLicensed), /授权已到期/);
  const undatedLicensed = { ...scale, provenance: 'licensed' };
  assert.throws(() => assertUsableScale(undatedLicensed), /授权量表必须登记/);
  const malformedLicensed = { ...scale, provenance: 'licensed', licenseExpiresAt: 'not-a-date' };
  assert.throws(() => assertUsableScale(malformedLicensed), /到期时间无效/);
