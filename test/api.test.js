...
test('professional review, assignment, acknowledgement and independent closure work', async () => {
  ...
  assert.equal(Object.hasOwn(review.body.data, 'tenantId'), false);
  assert.equal(Object.hasOwn(review.body.data, 'signalIds'), false);
  assert.equal(Object.hasOwn(review.body.data, 'closureReasonCiphertext'), false);
  ...
  assert.equal(Object.hasOwn(assign.body.data, 'signalIds'), false);
  ...
  assert.equal(Object.hasOwn(ack.body.data, 'tenantId'), false);
  assert.equal(Object.hasOwn(ack.body.data, 'userId'), false);
  ...
  assert.equal(Object.hasOwn(leadFollow.body.data, 'noteCiphertext'), false);
  assert.equal(leadFollow.body.data.hasNote, true);
  ...
  assert.equal(Object.hasOwn(closure.body.data, 'closureReasonCiphertext'), false);
  ...
  assert.equal(Object.hasOwn(approved.body.data, 'closureRequestedBy'), false);
  assert.equal(Object.hasOwn(approved.body.data, 'closureReasonCiphertext'), false);
...
test('imports, governed schemas, aggregate analytics, exports and public content are controlled', async () => {
  ...
  try {
    ...
    process.env.CAMPMIND_MEDIA_SCANNER_READY = 'true';
    const missingAdapter = await request('/v1/media-assets', { method: 'POST', headers: auth(professional), body: JSON.stringify({ filename: 'missing-adapter.png', mediaType: 'image/png', base64: 'iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII=' }) });
    assert.equal(missingAdapter.response.status, 503);
    assert.equal(missingAdapter.body.error.code, 'MEDIA_SCANNER_NOT_CONFIGURED');
  } finally {
