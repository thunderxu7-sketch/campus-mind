...
export const DEMO_IDS = {
  ...
  expressionPolicy: 'expression-policy-demo-v1',
  expressionNotice: 'expression-notice-demo-v1',
  visualNotice: 'visual-notice-demo-v1',
...
export function seedDemoState(): DatabaseState {
  ...
  state.expressionPolicies.push({
    id: DEMO_IDS.expressionPolicy,
    tenantId: DEMO_IDS.tenant,
    schoolId: DEMO_IDS.school,
    enabled: true,
    visualEnabled: false,
    policyVersion: 'expression-policy-demo-v1',
    selfExpressionNoticeVersion: 'expression-demo-v1',
    visualNoticeVersion: 'visual-demo-v1',
    counselorIds: ['user-counselor-demo', 'user-professional-demo'],
    backupProfessionalLeadId: 'user-professional-demo',
    serviceHoursText: '合成演示服务时段：工作日 09:00–17:00。',
    contactInstructions: '合成演示不会提供即时救助；需要立即帮助请联系学校人工支持。',
    ackTargetMinutes: 60,
    entryRetentionDays: 30,
    closedRequestRetentionDays: 90,
    accessLedgerRetentionDays: 180,
    maxShareHours: 168,
    maxOpenRequestsPerStudent: 3,
    minAge: 12,
    maxAge: 17,
    createdAt,
    approvedAt: createdAt,
  });
  state.expressionNotices.push(
    {
      id: DEMO_IDS.expressionNotice,
      tenantId: DEMO_IDS.tenant,
      schoolId: DEMO_IDS.school,
      purpose: 'self_expression',
      version: 'expression-demo-v1',
      title: '表达与支持说明（合成演示）',
      body: '你可以记录想法，并自主选择是否分享给学校心理支持人员。该功能不用于诊断，也不替代紧急帮助。',
      approved: true,
      createdAt,
    },
    {
      id: DEMO_IDS.visualNotice,
      tenantId: DEMO_IDS.tenant,
      schoolId: DEMO_IDS.school,
      purpose: 'visual_interaction',
      version: 'visual-demo-v1',
      title: '可选视觉互动说明（合成演示）',
      body: '视觉互动仅在设备本地驱动中性化界面反馈，不上传视频、照片或面部关键点，也不推断情绪。',
      approved: true,
      createdAt,
    },
  );
